...
| A telefon „Kapcsolat tesztelése" gombja HTTP **404**-et ad | a **Vezérlő szerver** mezőbe az admin OLDAL címe került (`…/admin`) | a mezőbe az ALAP-cím kell (`https://live.pelda.com`) — az app fűzi hozzá a `/api/session/ping`-et. 1.0.019 óta mentéskor magától levágja, a szerver pedig induláskor és a Streamkulcs fülön is szól, ha a `.env`-ben útvonalas cím áll |
| A WHIP publish HTTP **404** — a publish cím `…/ingest/onlive/whip` alakú | az **ingest alap-címben** bennmaradt egy útvonal (`/ingest`) | a MediaMTX a WHIP-et a saját portjának **gyökerében** szolgálja ki: az alap-cím `http://<gép>:8889` vagy `https://ingest.pelda.com`, útvonal nélkül. 1.0.103 óta az app mentéskor levágja, és a mező alatt előre szól |
| A „Helyi elérés" cím nem válaszol, pedig a szerver fut | tűzfal: a 8080/8889 bejövő nincs átengedve | Windowson rendszergazdaként: `netsh advfirewall firewall add rule name="OnLIVE 8080" dir=in action=allow protocol=TCP localport=8080` és ugyanez 8889-re (`protocol=TCP` és `protocol=UDP` is kell a médiához). A szerver induláskor kiírja a saját helyi címeit — böngészőből nyisd meg a telefonon: `http://<cím>:8080/admin` |
| A telefon végtelenül „Újracsatlakozás…"-t ír, pedig a szerver látja a kapcsolatot | a **vezérlés** és a **WHIP publish** két külön út: az egyik megy, a másik nem | 1.0.102 óta az app kiírja mindhárom láb állapotát (vezérlő szerver / WHIP publish / a szerver lát-e képet) és a hiba szövegét is. HTTP 404 a WHIP-en = rossz ingest cím (a MediaMTX 8889-es portjára kell mutatnia) |
...
| A telefon nem tud publikálni, pedig a vezérlő szerver válaszol | az **ingest** cím ugyanarra a hostra mutat, mint a vezérlő szerver | a WHIP a MediaMTX **8889**-es portjára megy, a vezérlő szerver a 8080-asra: külön tunnel-hostname kell (`ingest.pelda.com → http://localhost:8889`). A cloudflared NEM vág le útvonal-előtagot, ezért a `…/ingest/onlive/whip` alak nem működik |
